...
class RealisticPumpSimulator:
    ...
    def _create_pump_characteristics(self):
        ...
        # BEP sensitivity: 1.0 Hz per 1m (5 Hz per 5m)
        # At rated_head=30m, BEP=45Hz
        # At 35m: 45 + (35-30)*1.0 = 50Hz
        # At 40m: 45 + (40-30)*1.0 = 55Hz
        rated_head = 30.0  # Reference head
        base_bep_freq = 45.0  # BEP at rated head
        bep_sensitivity = 1.0  # Hz per meter
        
        self.bep_frequency = base_bep_freq + (self.system_head - rated_head) * bep_sensitivity
        
        # Efficiency decreases when operating away from rated conditions
        head_deviation = abs(self.system_head - rated_head) / rated_head
        base_efficiency = 0.78
        self.bep_efficiency = base_efficiency - 0.15 * head_deviation
